...
| `0x004DDC40` | `FootClass__ShouldBeOnBridge` | Early-out on `Foot+0x684 >= 0`, else delegates to the Object version. Virtual, slot **+0xBC** (proved from 5 constructors). **Role corrected 2026-08-19: this is not the cell-boundary transition.** Its candidate coord is `FootClass__GetDestinationCoords` 0x004DBDF0 (NavCom destination / tube exit) and every consumer is a `Can_Reach_Zone` 0x0056D100 query | per reachability query, not per cell |
| `0x005F6A70` | `ObjectClass__ShouldBeOnBridge` | Compares ground height at Location vs **destination** coord against `3 * g_nFootLevelHeightLeptons` (104, `0x00AC13C8`) and the destination cell's `0x100` flag. Both terms are `CellClass__GetGroundHeight` 0x00578080 = terrain Z with slope interpolation, not the deck plane | as above |
| `0x0075C154` / `0x004B1807` / `0x004B2561` | on-bridge transition block (inline, three identical copies in `WalkLocomotionClass__ProcessMovement` and `DriveLocomotionClass__Process_Drive_Track`) | **The actual cell-boundary predicate.** `MOVSX` both cells' signed level from `+0x11B`; enter on `dst == src - 4` AND `dst+0x140 & 0x100`; exit on `!(dst & 0x100)` AND `src & 0x100`. Writes `Foot+0x8C` at 0x0075C179 / 0x004B1830 / 0x004B2586. The boundary clear is `PerCellProcess`'s own tail through `ObjectClass__DropIn` 0x005F4160 (vtable+0xEC) | every cell-boundary crossing by a ground unit — very high |
...
| `0x004DDC40` | **Plate corrected, second pass.** The first version called this the cell-boundary transition predicate. It is not: its candidate coord is `FootClass__GetDestinationCoords` `0x004DBDF0` and its three verified consumers all feed `MapClass__Can_Reach_Zone` `0x0056D100`. The real transition is inline in the locomotors (`0x0075C154` / `0x004B1807` / `0x004B2561`), and the boundary clear is `ObjectClass__DropIn` `0x005F4160` via vtable`+0xEC`. Unverified-receiver callsites demoted to candidates |
| `0x005F6A70` | Plate added: destination-vs-Location ground-height comparison, `g_nFootLevelHeightLeptons` `0x00AC13C8` written at `0x005F37EB` with its 2x and 4x derivatives at `0x005F3831` / `0x005F3861`, and the note that `0x00578080` returns terrain Z with slope interpolation rather than the deck plane |
