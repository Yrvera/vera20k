...
- **Snapshot migration:** bincode encodes structs positionally, so new serialized VM fields cannot safely default from a shorter record. The initial registry payload bumped `SNAPSHOT_VERSION` 98 → 99; retaining resolved ScriptType/TaskForce provenance bumped 99 → 100; retaining the typed AITrigger payload bumped 100 → 101; removing the falsely retained token-4 scalar bumps 101 → 102. Older bytes are rejected cleanly, and v102 saves carry the corrected resolved registry.
...
- `EXACTIFICATION-RESIDUAL` — all 66 retail TaskForce member identities resolve through Infantry/Unit/Aircraft registries and explicitly author `TechLevel`, so the stock threshold oracle is exact. Custom missing-key data remains divergent because native initializes `TechnoType+0x634` to `255` while Rust's general `ObjectType` parser defaults it to `-1`; TaskForce family restriction is also not yet enforced at this boundary. Trigger: custom TaskForce members with missing `TechLevel` or BuildingType identities. Player effect: different trigger threshold or invalid-member admission. Frequency: zero in retail AIMD/rulesmd. [GHIDRA `0x00711082`, `0x00714570..0x00714584`, `0x006E8780`; retail corpus census]
...
Resolved definitions and order vectors serialize with `TeamScriptVm`. Because bincode cannot safely default a changed positional struct, the initial registry payload bumped `SNAPSHOT_VERSION` 98 → 99, resolved ScriptType/TaskForce provenance bumped 99 → 100, the typed AITrigger payload bumped 100 → 101, and removal of the non-native token-4 scalar bumps 101 → 102; older bytes are rejected at the envelope boundary. The custom Team CRC remains unchanged because the verified native CRC includes live Team/Script instance state, not the source registry. Tests must prove save/restore retains counts, order, IDs, and definitions.
