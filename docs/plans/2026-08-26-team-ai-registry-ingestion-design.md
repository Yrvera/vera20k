...
- `RESOLVED` — all 66 retail TaskForce member identities resolve through Infantry/Unit/Aircraft registries and explicitly author `TechLevel`. The Stage-A critic exactification also closes custom data: Rust now preserves category-distinct type identities, resolves TaskForce members in native Infantry/Unit/Aircraft order without BuildingType admission, and retains the native missing-key `TechLevel=255` constructor value while keeping explicit `-1` distinct. Both mode-zero and nonzero threshold folds are covered. [GHIDRA `0x004C4EF0`, `0x00711082`, `0x00714570..0x00714584`, `0x006E8780`; retail corpus census]
