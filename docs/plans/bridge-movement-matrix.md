...
|---|---|---|---|---|---|---|---|---|
| T1-01 | Hover | high intact | onto | move | **KNOWN-BROKEN**, re-measured | BayOPigs.mmx, approach `(111,152)` → deck `(111,151)`, LOOSE | OI-06 (=L1+L2) | **Measured 2026-08-27 — the defect SURVIVES the deck exemption.** This row asked for a re-run because its verdict predated `cf91caa3`. Done: `hover_tank_is_currently_blocked_from_entering_a_high_bridge` was re-run on HEAD *after* both `cf91caa3` (deck cells exempt from the corridor gate) and `c3c1cda2` (ordinary undisabled orders only), and it still passes — and because it is a characterization test asserting the broken behavior, passing means the ROBO still never leaves its cell. So whatever stalls a hover mover is **not** the corridor gate that was stopping ground units, and the two defects are unrelated. Now the active slice; diagnosis at `docs/plans/bridge-hover-t1-01-diagnosis.md`. |
