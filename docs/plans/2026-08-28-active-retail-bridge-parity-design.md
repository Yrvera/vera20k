...
**Revision 5 after the latest REVISE design-review verdict; pending a new fresh re-review. No Rust implementation is authorized by this document yet.**
...
The fourth review's random-map blocker and the Revision-4 critic's launch/projection blockers are
closed by
`docs/research/bridges/00-system-models/RMG_BRIDGE_DUAL_RNG_LIFECYCLE_REINVESTIGATION_GHIDRA_REPORT.md`
at commits `7fee6929` and `4a63fa15`. It proves that preview and gameplay are separate generation runs, that every
...
and never pass through fixed-map `OverlayClass::Mark`. No open or deferred lifecycle term remains.
...
2. Each Generate call begins with a fresh MapGen object seeded from `MapSeed+0x74`; it never starts
   ...
   identity, and `Discarded` or `Emitted { entity_index }` outcome. Successful output rows alone are
   not a sufficient reconstruction.
...
7. Launch replay builds a `GeneratedTechnoInitTable` keyed by stable generated entity index. A
   ...
   on `GameEntity`, and performs no second draw. Authored-map Technos retain their ordinary
   constructor-draw path. The field participates in deterministic snapshots and hashes.
...
struct RmgConstructionEvent {
    ...
    cell: CellCoord,
    outcome: RmgConstructionOutcome, // Discarded | Emitted { entity_index }
}
...
The 21 units below are implementation boundaries, not new architecture layers.
...
| 2 | Active RMG preview/accept/`.SED` launch lifecycle, low deck/end/CABHUT production, and waterfall-topology exclusion | BR-M02, BR-M03 | unit 1; `7fee6929` plus `4a63fa15`; fresh MapGen per run; first-entry/re-entry and no-preview acceptance gates; `RmgConstructionTrace`; one launch `ScenarioBootstrapRng`; validated `GeneratedTechnoInitTable`; complete stamped launch output; no generated Mark replay; `BuildRiverBridge` negative characterization |
| 3 | Fixed-map low overlay procedural load and Road mutation | BR-M05, BR-M11 | unit 1; Lost Lake/Killer plus destroyed low fixture; generated-source bypass preserving its full direct deck payload and zero Mark draws |
...
overlay replay are not left as new open questions. They are resolved before implementation by the
exhaustive lifecycle report at `7fee6929` plus follow-up `4a63fa15`, and are mandatory unit-2/unit-3
contract inputs.
...
The full suite `cargo test -p vera20k --lib` runs exactly once after all 21 units and their critic cycles pass, immediately before the bridge-wide reverse audit is declared ready. It is not rerun per unit.

## Bridge-Wide Reverse Audit

After all unit passes:
...
- every behavior claim is cited to the frozen native/retail coverage report, the closed dual-RNG
  lifecycle/follow-up report at `7fee6929` and `4a63fa15`, or explicitly open;
