...
- **Confirmed open gameplay callbacks (2026-09-12 correction)** — `0x00487A10(0)` runs after each changed strip's three Recalcs and synchronously checks/damages ground occupants and nearby locomotor users whose `Is_At_Coord` matches the repaired Cell. It is not visual bookkeeping. `0x005868A0` enumerates the completed rectangle into two-pass batch `0x00586990`; full base connectivity is a separate preceding callback. See the dated correction in [the walker report](../05-damage-collapse-repair-cabhut/REPAIRBRIDGEWALKER_BODIES_GHIDRA_REPORT.md). The legacy deferred/runtime-only Rust path still needs these live callbacks before engineer repair can close.
