...
5. Runtime placement cleanup now visits `N,E,S,W,self`, emits tactical then radar before each visit's wall gate, publishes each wall Recalc/conditional zone step in visit order, runs the explicit anchor Merge/graph before owner assignment, increments the source afterward, and runs the common anchor Recalc second. Each clicked/filler cell completes this transaction before the next scan/stamp.
