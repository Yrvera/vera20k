...
8. Build-rate / unit-rate timers, defeat detection: when owned buildings+units hit zero (and not observer), `HouseClass__ScatterAllUnits` + `HouseClass__MPlayer_Defeated`.
...
- Player-visible outputs every match: low-power EVA, super-weapon ready flash, insufficient-funds / silos-needed EVA cues, AI opponents building and producing, and defeat handling (units scatter + "player defeated"). These are exactly the kind of outputs the parity bar covers.
