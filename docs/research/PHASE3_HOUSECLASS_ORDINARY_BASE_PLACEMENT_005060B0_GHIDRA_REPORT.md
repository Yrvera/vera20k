...
`HouseClass__RecenterBase @ 0x0050C210` is the ordinary active writer of `House+0x5750`. It scans `House+0x6C/+0x78` in reverse owned-object order and selects the first object whose type is in `BuildConst` and whose object byte `+0x81` is clear. It converts that object's lepton X/Y to signed-truncating cells, writes the cell to primary base cell `+0x5490`, refreshes AI build options only when the base-plan node count `+0x5714` is zero, writes the cell into the first node's cell slot `(*(House+0x5708)+4)`, and finally writes `+0x5750`. If no eligible BuildConst object exists, it leaves those cells unchanged. The successful non-player-control ConstructionYard deploy branch at `0x007398DE..0x007398F3` also writes its new yard cell to the first plan node and `+0x5750`.

Current Rust deliberately keeps `HouseState.base_center` as the assigned launch/primary center and does not recenter it during ordinary structure lifecycle. That is correct for the existing `+0x5490` use but cannot stand in for the distinct base-plan center. Implementation therefore needs separate base-plan-center state and the verified reverse-order BuildConst recenter writer; it must not change launch-center semantics to make this reader convenient.
...
The primary ordinary-skirmish writer is `HouseClass__AI_RecalcBuildOptions @ 0x005054B0`, not `AI_Manage_Build_Queue` and not a generic next-building chooser. `UnitClass__Deploy @ 0x00739380` reaches it through the sole caller `FUN_00505180 @ 0x00505180` when all of these gates hold: deployment successfully created the Building, the owner is not human-controlled, the deployed BuildingType is a ConstructionYard, and `g_GameMode != 0`. `FUN_00505180` calls `Computer_Paranoid` for the ordinary non-player nonzero-mode case, saves `g_MapEditorMode`, and calls Recalc with editor mode forced to zero only when `House+0x5714 == 0`, then restores the mode. The deploy path writes the new yard cell into node zero, `House+0x5750`, and the related AI-ready flags after Recalc.

`HouseClass__RecenterBase @ 0x0050C210` is the second ordinary caller. After selecting the last live owned BuildConst object in reverse owned-object order, it uses the same empty-count gate and mode save/restore, then writes that yard cell into node zero and `House+0x5750`. The only other Recalc xref is `HouseClass__ComputerTakeover @ 0x0050A5C0`, addressed separately below.
...
The active YR retail inputs are source-ordered `BuildConst`, `BuildPower`, `BuildRefinery`, `BuildBarracks`, `BuildTech`, `BuildWeapons`, `BuildRadar`, and `HarvesterUnit` lists; `HarvestersPerRefinery=2,2,1`, `AISlaveMinerNumber=4,3,2`, and defense counts `25,20,6` Allied, `25,22,6` Soviet, and `25,22,6` Third. Their difficulty order is native Hard/Normal/Easy. None of these values may be replaced by Rust's generic infrastructure priority list.
...
| last building attack frame | `HouseClass__Constructor @ 0x004F5A59` initializes signed `House+0x54D8` to zero. A full-program instruction search found exactly one non-constructor writer: `BuildingClass__ReceiveDamage @ 0x0044229C`. When attacker is non-null and victim virtual `+0x80` is false, it writes `g_CurrentFrameCounter` before immunity checks and before the generic receiver, regardless of alliance, damage sign, or eventual result. The other native references are two reads in `HouseClass__AI_Building_Strategy` and this helper read | Rust's receiver owns entity `last_attacker_id` and `was_attacked_by_enemy`, but no snapshot/hash-covered House last-building-attack frame and no equivalent pre-receiver writer |
...
| `HarvestersPerRefinery=2,2,1` and `AISlaveMinerNumber=4,3,2` | active Hard/Normal/Easy vectors in `rulesmd.ini` | generated refinery duplicates and their shared-RNG insertion count are active |
...
3. Add a distinct base-plan center and implement the `0x0050C210` reverse-owned-order live BuildConst recenter writer plus the active AI ConstructionYard-deploy write. Preserve the existing launch/primary base center as separate state.
...
| live decompile/assembly `FUN_00505180 @ 0x00505180`, `UnitClass__Deploy @ 0x00739380`, `HouseClass__RecenterBase @ 0x0050C210`, `HouseClass__AI_RecalcBuildOptions @ 0x005054B0`, `FUN_00505360`; Rules reader `BuildRadar` binding; retail Rules lists/count vectors | fresh-skirmish/recenter population authority, exact eligibility and priority topology, prerequisite tokens, refinery/defense insertion counts, shared Scenario RNG ownership, node order and zero initialization | HIGH |
| House constructor strategy-timer stores `0x004F5B9D..0x004F5BA8`; live decompile/assembly `HouseClass::Update @ 0x004F8B50` strategy-timer block `0x004F8FBE..0x004F9032`, `HouseClass__AI_Building_Strategy @ 0x004FD500`, `HouseClass__AI_Check_Build_Need @ 0x004FD9A0`, `FUN_004F6540`, `HouseClass__AI_Manage_Build_Queue @ 0x004FDD10`, insertion blocks `0x004FE163..0x004FE223/0x004FE2E7..0x004FE3A6`; `HouseClass__ComputerTakeover @ 0x0050A5C0` | immediate-expiry initialization, signed AI/non-passive cadence, exact AI-hate/SW/planning/reschedule order, one unconditional 106..112 reschedule draw after earlier variable SW draws, complete support/factory trigger, nonzero-mode runtime insertion, funding/candidate branches, index-zero quirk, capacity fallback, exact inserted node fields/order, and evidence-backed takeover boundary | HIGH |
