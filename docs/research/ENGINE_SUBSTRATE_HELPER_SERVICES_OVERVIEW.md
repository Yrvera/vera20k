...
- **Drawing — (withdrawn 2026-09-07) normal building bodies DO write `g_ZBuffer` per pixel via the BUILDNGZ z-shape (`0x004990e0`), and other objects Z-test per pixel without writing** → a unit can be partially hidden behind a building's tall part; unit-vs-unit occlusion is painter's-order only (see ZBUFFER_DEPTH_SYSTEM.md Overview); Air-layer objects (aircraft, in-flight missiles, jumpjet) draw in submission order, not depth-sorted (D8b).
