...
- INI/source checked: `ini/rulesmd.ini`, `ini/rules.ini`, `ini/artmd.ini`, `ini/art.ini`; retail path grep under `<ra2-install>/`; `src/rules/object_type.rs`, `src/app_ui_overlays.rs`, `src/app_selection_brackets.rs`, `src/app_render/build_instances.rs`.
