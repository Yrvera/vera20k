...
| BUILDNGZ / Z-shape blitter | (corrected 2026-09-07) the normal body route with flags `0x6E00` selects slot `+0x158` → `0x004990e0` (extended) / `+0xbc` → `0x004958d0` (standard), both of which Z-test per pixel and write `g_ZBuffer`; `0x00497100` is the extended selector's default arm, not this route | `TechnoClass_DrawSHP @ 0x00705E00` ORs `0x4000` when arg 9 != 0 and `BuildingClass_DrawBody` passes 1; leaves read from bodies | Active for normal building body (see ZBUFFER_DEPTH_SYSTEM.md Overview) |
...
[REOPENED 2026-09-07] OQ-5 - Does normal building SHP body populate the bracket-sampled depth buffer? The earlier "No" was wrong: it checked the cloak-mode z-bits instead of the `0x4000` Z-write bit. The normal body route (flags `0x6E00`) reaches `0x004990e0`/`0x004958d0`, which per-pixel test `base_z - zshape < zbuf` and write `g_ZBuffer`. So yes, the normal building body writes the depth buffer the bracket samples. See ZBUFFER_DEPTH_SYSTEM.md Overview for the traced slots and leaves.
