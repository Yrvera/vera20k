...
| Conditional cleanup count reversal | Implemented through `OverlayGrid::recalculate_runtime_cell` and its `zone_changed` outcome | A cleanup-removed source reverses its eight wrapping count writes only when Recalc changed reduced zone type. House sale intentionally leaves the sold source stale. |
