...
- `docs/research/traces/SKIRMISH_BUTTON_TEXT_RECTS_PRESSED_OFFSETS_800X600_TRACE.md` replacement wording: "Superseded for current Rust status by `SKIRMISH_OWNERDRAW_BUTTON_PIXEL_RECHECK_800X600_GHIDRA_REPORT.md`: current `button_text_rect` keeps binary right/bottom edges and `push_button_label_draw` uses yellow enabled text. Keep the binary facts, but remove the current-Rust FAIL verdicts."
