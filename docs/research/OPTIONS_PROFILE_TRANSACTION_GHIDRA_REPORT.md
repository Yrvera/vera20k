...
**Claimed Scope:** the active-YR process-owned `OptionsClass` construction, one-profile read/apply, dialog-to-object updates, and serialized `RA2MD.INI` write transaction for persisted `[Options]`, `[Video]`, and `[Audio]` settings.
...
The Phase 14 implementation gap is not six isolated checkbox keys or one music-volume key. The missing mechanism is a process-owned profile transaction: one authoritative in-memory value set, one load boundary, explicit application to existing runtime consumers, and one byte-preserving commit boundary. The current Rust implementation instead parses/discards startup video values, hydrates only two `[Options]` fields, reads only `ScoreVolume`, and writes settings from several unrelated paths.
...
Later, active `Init_Game @ 0x0052BA60` loads MIX/campaign substrate, calls `OptionsClass__ReadFromINI` at the `0x0052C630` function body, and only then constructs tactical/audio-list consumers. This is a process-load operation, not a simulation tick, replay datum, or save-state field.
...
- `[RESOLVED] OQ-04 — When does the screen pair affect startup? -> WinMain consumes it before display creation; either -1 replaces the pair together.` (evidence: `0x006BD94A..0x006BD9B5`)
...
- `[RESOLVED] OQ-06 — When does the full read run? -> Init_Game after MIX/campaign load and before later tactical/audio-list initialization.` (evidence: `Init_Game @ 0x0052BA60`, call in body beginning `0x0052C630`)
...
- `[RESOLVED] OQ-25 — What happens on an empty profile? -> Constructor defaults survive, with read-time side effects applying those defaults.` (evidence: SetDefaults + current-as-default Read calls)
...
1. If only one screen dimension is missing, both dimensions fall back together (OQ-04).
...
|---|---|---|---|---|---|---|---|
| milestone-blocking | one process-owned Options/Video/Audio profile is defaulted, loaded once, applied to consumers, and committed as one settings transaction | `0x005FA350`, `0x005FA620`, `0x005FAD10` | missing; several partial readers/writers | `src/app/`, existing persistence/options and startup-options surfaces | introduce one process-lifetime profile authority using existing typed INI semantics and byte-preserving writer; retain it across shell/match transitions | absent `RA2MD.INI` yields exact defaults; a populated profile hydrates every modeled field; one quit commit preserves unrelated bytes and writes native spellings | do not create a second parser, duplicate field owners, or put profile state in `sim/` |
| milestone-blocking | early interactive screen pair is consumed before window creation and reused for shell mode; either sentinel replaces the pair | WinMain `0x006BD94A..0x006BD9B5` | parser exists but production discards it; window fixed 800x600 | `src/main.rs`, `src/app/mod.rs`, `src/app/initialize.rs`, `src/app/handler.rs` | retain startup/profile resolution for interactive mode while preserving capture dimensions as higher-priority automation input | profile 640x480 creates/re-enters 640x480 shell; one missing dimension selects pair fallback; capture still uses requested dimensions | do not let profile dimensions alter sealed capture contracts or deterministic render dimensions without their existing override |
...
| compounding | accepted in-game dialog applies before write; canceled/terminated result writes nothing | `0x004E1D00`, `0x004E1DE0` | six-key close writer exists, fragmented from profile | `src/app/persistence/options.rs`, in-game dialog close path | mutate process profile through UI view, apply consumers, then commit once only on accepted result | accept changes fields/effects/file in order; cancel leaves object, effects, and file unchanged | do not serialize directly from transient widget state through repeated per-key filesystem writes |
