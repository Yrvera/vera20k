...
4. Computes remaining capacity as `Storage - current_load`.
5. Converts remaining capacity through `Math__ftol`.
6. Calls `CellClass::Reduce_Tiberium(amount)`.
...
- Empty `CMIN`: `Storage=20`, total `0.0`, request `20`.
- `CMIN` with 19 carried units: request `1`.
- `HARV` with 38 carried units: request `2`.
...
| Request amount is truncated remaining capacity in tiberium units, then `Reduce_Tiberium` clamps to cell content. | `0x0073D450`; `0x007C5F00`; `0x00480A80` | mostly implemented for integer cargo; fractional storage cannot be represented by `Vec<CargoBale>` | `Miner::cargo`, `extract_bales_max` | Preserve integer cases exactly; if future fractional storage is modeled, request should truncate toward zero and request 0 when less than one unit remains. | `partial_cargo_request_truncates_remaining_capacity`: with capacity 20 and 19 carried units, request/extract at most 1; with a future fractional 19.5 storage harness, request 0. | Do not round to nearest or ceil remaining capacity. |
...
- Do not use `ceil` or round-to-nearest for remaining capacity; the request conversion is `Math__ftol` truncation.
- Do not assume Rust tests that insert `remaining = 11 * 120` prove parity for real map overlay `OverlayData=11`.

## 11. Remaining Uncertainty

- The x87 instruction-level sequence around the decompiler's `remaining <= epsilon` comparison was not disassembled in this slot. Normal integer cargo cases are still high confidence because the direct path and prior docs agree on `Storage - total`, `Math__ftol`, and `Reduce_Tiberium`.
