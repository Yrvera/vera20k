...
resumption. Dependencies are generated by `cargo modules dependencies`; rows are
...
private modules. Each row describes that module, not all its descendants.
...
bug. If a row is absent, check its parent and the current source/build
configuration instead of guessing a dependency.
...
# Regenerate and compare dependencies without rewriting this file:
python tools/module_map.py --check
```

The formatter runs the existing tool command below and updates only the marked
...
or target; the saved map describes only the recorded build scope.
