...
| `CellClass::RecalcAttributes` automatic TubeClass branch in `docs/research/bridges/04-locomotion-height-tubes/LOW_BRIDGE_TUBECLASS_GHIDRA_REPORT.md` | Primary native | Shell eligibility is LandType 10, no existing tube index, and tile membership in one of the four fixed inclusive `base..=base+3` bands for Tunnels/TrackTunnels/DirtTunnels/DirtTrackTunnels; the tile's zero-based offset within the matched band indexes `[2,4,6,0]`, and shells are same-cell zero-step records. Native does not consult `TilesInSet` after the cumulative base is loaded. |
...
| Six active MD theater INIs loaded from the installed retail MIX chain | Primary retail | All six active theaters provide the same exact ten relative values; configured tunnel-set bounds are the corpus input |
| Installed retail MIX chain under `C:\Users\enok\Documents\Command and Conquer Red Alert II` | Primary retail | Exact ten-key vectors through the normal loader plus automatic-shell corpus result: 64 unique fixed-band base IDs, eight absent base files, 56 present bases plus ten variants = 66 loaded TMP assets, 731 present subcells, and 36 qualifying subcells |
...
The retail corpus was read through the repository's released asset loader. It enumerated the union of each configured set's fixed `base..=base+3` tile-ID band, including aliases caused by zero-length sets, and the loader's actually present contiguous variants, then decoded every present TMP subcell through the active terrain-byte mapping.
...
TEMPERATE, SNOW, DESERT, every fixed-band alias, and every other active set in NEWURBAN contribute zero qualifying subcells. The four LUNAR keys declare zero-length sets but share a cumulative base that aliases `Green01` and `glat01..03`; those files and their variants are present but contribute no qualifying subcells.

The fixed native bands address 64 unique base tile IDs. Eight corresponding base filenames are absent from the shipped MIX chain: `SNOW/MDrodc01..04.sno`, `URBAN/MDrodc01..02.urb`, and `NEWURBAN/MDrodc01..02.ubn`. The other 56 bases are present, and the loader discovers ten additional LUNAR variants: `Green01a..g.lun` and `glat01a.lun`, `glat02a.lun`, `glat03a.lun`. The complete decoded aggregate is therefore 66 loaded TMP assets, 731 present subcells, and exactly the 36 LandType-10 shell candidates listed above.
...
|---|---|---|---|---|---|---|---|
| U1-01 | REQUIRED_FIX | MILESTONE | Ten theater bridge-piece inputs | Preserve all ten distinct BridgeSet-relative integer values, including independently configured duplicates and absent/`-1` values. | Six top/middle fields are parsed, the middle pair is narrowed to `u8`, and four bottom fields are discarded. | Store all ten optional inputs in the existing `u16` tile-index domain, parse the exact key names, and update every construction boundary without changing ramp-table membership. | Deterministic synthetic fixtures prove ten independent fields, a middle value above 255, and absent/`-1` behavior; the ignored normal-loader retail oracle proves the six active theater vectors are all `1,2,3,3,4,5,6,6,7,12` without a compile-time dependency on developer-local INIs. |
...
| U1-04 | TEST_ONLY | CONTENT-CONDITIONAL-BLOCKING | Automatic-shell predicate | Final LandType must be 10, tile must be in one fixed `base..=base+3` band supplied by a configured tunnel family, and an existing tube index suppresses shell creation. Tile offsets `0..3` map exactly to directions `[2,4,6,0]` for every family. The comparison does not use the configured set's count, so a zero-length set still contributes its cumulative base. | Predicate and directions appear implemented, but only partial synthetic coverage exists and the count-independent zero-length alias is untested. | Preserve the fixed-band implementation and add exact synthetic boundary tests for byte-5 mapping, four-entry bounds, per-offset directions, every family, zero-length-base aliasing, and existing-tube suppression. | Tests reject wrong land, fifth tile, absent family, and existing tube; accept every offset/family combination and a zero-length-base alias with exact same-cell endpoints and direction. |
| U1-05 | TEST_ONLY | MILESTONE | Shipped automatic-shell corpus | The active six-theater fixed bands cover 64 unique base IDs: eight base files are absent, 56 bases plus ten variants load, 731 subcells are present, and exactly the 36 subcells listed above qualify. | No executable retail corpus oracle exists. | Add an ignored retail-data test using the normal theater/asset/TMP loaders. It must enumerate the fixed native bands, zero-length aliases, missing files, loader-visible variants, and decoded subcells rather than hard-code only positive filenames. | With configured retail data, the exact missing set, exact ten-variant set, 66/731/36 aggregate, and exact positive set match; no unlisted theater/file/subtile qualifies. |
...
- The existing `bridge_middle_1` and `bridge_middle_2` fields also become `Option<u16>`; there is no native byte truncation at the theater-input boundary.
...
2. **Six-theater retail INIs:** the ignored installed-retail normal-loader oracle proves each active MD definition produces the exact ten-value vector; the ordinary suite must not compile developer-local or gitignored INI paths.
...
5. **Automatic-shell retail corpus:** ignored external-data test proves 64 fixed-band base IDs, the exact eight missing base files, the exact ten loaded variants, 66 loaded assets, 731 present subcells, 36 candidates, and exactly the listed URBAN/NEWURBAN positives.
...
The corpus count treats a TMP subcell as present only when the retail loader exposes that subcell and decodes its terrain byte. It enumerates the union of each configured range's fixed first four absolute tile IDs and their contiguous loader-visible variants, matching the active shell predicate even when a zero-length set shares its cumulative base with a later set. The eight missing bases and ten present variant siblings are therefore measured corpus facts, not filename assumptions.
