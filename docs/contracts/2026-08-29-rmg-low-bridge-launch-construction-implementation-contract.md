...
- The tile-block stamper uses scratch id `-1`, level base `-1`; it changes tile/sub-tile/slope and
  scratch tag but preserves cell level.
...
| U2-07 | TEST_ONLY | COMPOUNDING | Seed-cell rejection picker | Per attempt, whole-square uniform draws reject wrong region and `(0,0)`, with each rejection spent. | `bridge_deck.rs:29-116` implements this behavior with a non-production spin escape and focused cursor tests. | Preserve logic; make it the actual placer entry. The escape may remain only as a VERA nontermination guard for impossible no-cell fixtures. | closure report §4.1; current Rust | Existing rejection/cursor tests remain; integrated placer fixture proves the accepted seed and post-rejection cursor. |
...
| U2-12 | REQUIRED_FIX | MILESTONE-BLOCKING | Conditional paved-end coins and block stamps | Per end, draw `U{0,1}` only after validator success; alternates PavedRoads `+10,+9,+13,+12`, defaults PavedRoadEnds `+0,+2,+1,+3`; exact anchors; scratch/level args `-1`; level preserved. | No end selection or stamping exists; `TileIds`/TMP blocks already carry both bases and subcell layouts. | Implement exact per-axis order, conditional draws, identities, anchors, and a reusable native-equivalent block stamp that writes tile/sub-tile/slope and `scratch.stamp=-1` without changing level. | closure report §7; retail theater data | All four ends cover validator false, coin false, coin true; assert draw counts, exact anchors/tiles/subcells/slopes, scratch tag, and level preservation. |
