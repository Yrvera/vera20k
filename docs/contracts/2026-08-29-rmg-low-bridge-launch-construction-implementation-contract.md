...
| U2-20 | TEST_ONLY | COMPOUNDING | Waterfall terrain no-topology boundary | `BuildRiverBridge @ 0x0059E740` changes tile/subtile/slope/level/scratch waterfall terrain only; no low overlay/data/raw flags/Tube/CABHUT/trace. | The waterfall and shore block stampers now retain each TMP subtile slope. A fixed-seed direct production-owner test proves all allowed terrain changes, the complete negative topology boundary, generated materialization with zero raw bridge flags/deck/tubes, and identical repeat output/MapGen continuation. | Closed; preserve both slope writes and the direct negative characterization. | closure report §10; current Rust | Deterministic success snapshot asserts allowed terrain changes and unchanged overlay, density, raw bridge flags, explicit tubes, structures, and trace. |
| U2-21 | DOC_ONLY | COMPOUNDING | Active/dormant naming boundary | RMG is conditionally active through stock Create Random Map/`.SED`; `BuildRiverBridge` is waterfall terrain despite its name; dormant helpers/TrainBridgeSet remain excluded. | Module/API comments, tests, and private waterfall-control names distinguish conditional active RMG, low-deck construction, and waterfall terrain. Only misleading inherited native/API identifiers and explicit negative topology assertions retain bridge terms; excluded helpers remain absent. | Closed; preserve the source boundary and do not activate `TrainBridgeSet` or its helper family. | closure report §§1,10-11 | Diff/source review contains no dormant claim for active RMG, no TrainBridgeSet field, and no activation of excluded helpers. |
