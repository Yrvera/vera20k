...
| Source | Role | Use |
|---|---|---|
| active `gamemd.exe` functions `0x005FA350`, `0x005FA620`, `0x005FAD10`, `0x004E1D00`, `0x004E1DE0`, `0x0055FAA0`, `0x0055FC80`, WinMain `0x006BD94A..0x006BD9B5`, and the later full-read call at `0x0052C630` | PRIMARY | defaults, two-stage screen/full read ordering, apply/write ownership, clamps, screen fallback, dialog commit gates |
...
- Video width/height begin at `-1`. WinMain first reads only the screen pair and, if either remains `-1`, replaces both live fields before creating the display. The Rust-native ordinary-host fallback is the already documented built-in `800x600`; the low-memory `<= 0x200000` retail branch is not observable through wgpu and remains non-blocking platform exactification. `Init_Game` later calls the full reader at `0x0052C630`, which rereads the Video pair using the post-fallback object as its missing-key defaults. A width-only `640` profile therefore creates an `800x600` window but retains/writes `640x600`.
...
|---|---|---|---|---|---|---|---|
| `REQUIRED_FIX` | `MILESTONE-BLOCKING` | process profile ownership | one singleton holds defaults, loaded values, dialog mutations, and final write values across the whole process | `App` retains only `StartupAudioDisposition`; Options live under match presentation; Score lives in `MusicPlayer`; startup video is discarded | add one process-lifetime Rust profile owner with exact Options/Video/Audio fields; UI/audio/window state becomes a projection/consumer, not parallel authority | gamemd `0x005FA350/620/AD10`; `src/app/mod.rs:77-133`; `src/app/state.rs:15-33`; `src/ui/shell/in_game_options_state.rs:17-53` | absent-file construction preserves exact Options/Audio defaults and resolves the screen sentinel pair to the ordinary-host fallback; profile survives shell -> match -> shell without re-defaulting or losing untouched fields |
| `REQUIRED_FIX` | `MILESTONE-BLOCKING` | one parsed snapshot, two native read phases | WinMain performs an early screen-only read/fallback; the later full read uses the resulting current fields as typed-reader defaults for every owned field and reapplies any present Video key | startup video helper uses `get_i32` and has no production caller; boot reads only ScrollRate/DetailLevel; Score has a separate reader | read and parse one byte snapshot, apply the early screen pair and fallback to obtain the window projection, then apply the complete typed read again from the same parsed snapshot; apply only verified field clamps | WinMain `0x006BD94A..0x006BD9B5`; full reader `0x005FA620` called at `0x0052C630`; typed readers `0x005276D0/0x005295F0/0x005283D0` | typed fixture produces the contract values; width-only `640` produces startup window `800x600` and retained profile `640x600` without a second filesystem read |
| `REQUIRED_FIX` | `MILESTONE-BLOCKING` | early interactive screen size | early profile/switch screen fields are resolved before display creation; either sentinel replaces the live pair; the later retained profile may differ after its full Video reread | `RetailStartupOptions` contains the pair, but `App::new` discards it; `initialize` and `enter_shell_window_mode` hardcode 800x600 | retain the early effective shell size separately from the later full-read profile result, and use the early size on shell re-entry | WinMain `0x006BD94A..0x006BD9B5`; later full-read call `0x0052C630`; `src/app/frontend/startup_options.rs:98-203`; `src/app/initialize.rs:35-50`; `src/app/handler.rs:31-35` | complete 640x480 creates/re-enters 640x480; width-only 640 creates/re-enters 800x600 while retained/write state is 640x600; capture dimensions still win and remain hidden |
...
| `REQUIRED_FIX` | `COMPOUNDING` | accepted dialog commit ordering | accepted in-game dialog mutates live object/consumers before one write; pump/game-termination result 2 mutates/writes nothing | current close path applies/persists six fields, but writes directly from match UI state; launcher has no controls | route accepted changes into the process profile, apply projections, then commit; result 2 leaves profile, consumers, and bytes unchanged | `0x004E1D00`, `0x004E1DE0`; `src/app/persistence/options.rs:82-145` | accept changes profile -> target line/tooltip/detail effects -> file; result 2 produces no profile/effect/file delta |
...
| `TEST_ONLY` | `COMPOUNDING` | capture override isolation | retail has no capture harness; VERA capture dimensions are an authorized sealed automation override | `initialize` already prioritizes `capture_dimensions` over fixed shell size | preserve this priority and prevent every user-profile field from entering sealed capture state | `src/app/initialize.rs:35-49`; capture constructors/launch routing | a production-used pre-window selection seam proves a conflicting full-profile loader is never invoked, exact Options/Audio defaults remain projected, explicit capture dimensions win, and the capture window remains hidden |
...
2. Unified loader and startup ordering
   ...
   - Parse one immutable `IniFile`; apply its screen pair once for the WinMain-stage fallback/window result, then apply the full typed profile read from that same snapshot to the post-fallback profile.
   - Use existing typed `read_*` services and the verified per-field clamps in both semantic passes; do not perform a second filesystem read.
   ...
   - Move the later full-read profile into initialized app state while retaining the early effective window pair for shell re-entry.
...
2. `profile_load_uses_native_typed_semantics_and_clamps`
   ...
   - Action: read/parse one snapshot and run the early screen-only plus later full typed passes.
...
3. `interactive_profile_screen_pair_precedes_window_and_survives_shell_reentry`
   ...
   - Setup: complete interactive profile 640x480, width-only `640`, height-only `480`, argv-seeded missing-key variants, and a capture request whose injected user-profile loader would return conflicting screen/Options/Audio fields.
   - Action: resolve the early initial target, drive the production shell re-entry operation seam, inspect later retained state, and drive the production capture-selection seam.
   - Expected: complete pair uses/retains/re-enters 640x480; width-only uses/re-enters 800x600 but retains 640x600; height-only uses/re-enters 800x600 but retains 800x480; argv values survive absent keys; capture never invokes the user-profile loader, retains exact Options/Audio defaults plus the post-fallback 800x600 screen, uses explicit dimensions, and remains non-visible.
...
7. `accepted_and_result_two_options_transactions_are_atomic`

   - Setup: initial profile/consumers/file snapshot; one accepted result and one pump/game-termination result 2.
   - Action: close each dialog path.
   - Expected: accepted path updates profile, then consumers, then commits once; result-2 path changes none of them.
   - Proves: row 9.
