...
// Bumped 112 -> 113: the completed active-retail crate runtime persists its
...
// Bincode encodes these records positionally, so v112 cannot be admitted.
...
mod tests {
    ...
    /// Tactical camera presentation adjunct envelope.
    #[test]
    fn phase3_active_retail_crate_snapshot_version_is_113() {
    ...
    fn drive_ship_active_and_stashed_slope_phase_roundtrip_without_resample_or_rng() {
        ...
        let mut restored = GameSnapshot::load(&bytes)
            .expect("current v113 slope snapshot")
            .sim;
        let loaded = restored
            .substrate
            .entities
