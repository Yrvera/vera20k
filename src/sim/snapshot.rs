...
// v156 separately persists current-house process input on the main branch.
// v157 combines both layouts; earlier branch-local saves cannot be decoded as this schema.
const SNAPSHOT_VERSION: u32 = 157;
...
",
    ...
    fn current_snapshot_version_includes_gap_operational_state() {
        ...
        assert_eq!(super::SNAPSHOT_VERSION, 157);
    }

    #[test]
    fn combined_bridge_current_house_schema_rejects_separate_layouts() {
        for version in 153..=156 {
            let preamble = GameSnapshotPreamble {
                product_magic: SNAPSHOT_PRODUCT_MAGIC,
                envelope_version: SNAPSHOT_ENVELOPE_VERSION,
                version,
            };
            let bytes = bincode::serialize(&preamble).expect("branch-local header");
            assert!(matches!(
                GameSnapshot::load(&bytes),
                Err(SnapshotError::VersionMismatch { expected: 157, found }) if found == version
            ));
        }
    }
    ...
    fn current_house_is_saved_client_input_with_validated_reference() {
        ...
        for owner in owners.iter().copied() {
            ...
            sim.bind_notification_local_owner(Some("Second"));
            ...
            restored.bind_notification_local_owner(Some("First"));
            assert_eq!(restored.session.current_house, Some(owner));
            assert_eq!(restored.state_hash(), shared_hash);
