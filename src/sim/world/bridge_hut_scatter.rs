...
use crate::sim::movement::{self, ground_pose, locomotor::MovementLayer};
...
impl Simulation {
    ...
    pub(super) fn hut_infantry_coordinate(&self, id: u64) -> Result<DriveCoord, String> {
        let e = self
            .substrate
            .entities
            .get(id)
            .ok_or("hut listener disappeared")?;
        if let Some(state) = e.low_bridge_tube_state {
            ...
            let tube = self
                ...
                .ok_or("hut coordinate references a missing active TubeClass")?;
        ...
        let loco = e
            .locomotor
            .as_ref()
            .ok_or("hut coordinate requires the active Infantry locomotor")?;
