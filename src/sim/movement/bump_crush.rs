...
/// RESIDUAL (GSI-08.17) — the crush kill skips two consequences. A crushed
...
/// Terrorist or Demo Truck does not detonate. Crush kills also bypass
/// `capture_kill_credit`, so the crusher's house is not credited.
/// - Trigger: driving over any unit with a `DeathWeapon=` (13 stock types), and
///   every crush kill for the credit arm.
/// - Player effect: running over a Terrorist is free, which is exactly backwards
///   — retail punishes it — and crush kills do not count toward the score or
///   toward whatever a promotion system would award.
/// - Frequency: crushing is routine tank play; the `DeathWeapon=` arm is
///   bounded by those 13 types but includes the two units players most want to
///   crush.
/// - Downstream risk: firing the death weapon spawns an area-damage event from
///   inside the movement pass, so it needs the same ordering care as any
///   movement-time damage commit; the credit arm additionally feeds score and
///   the veterancy award this phase records as absent.
