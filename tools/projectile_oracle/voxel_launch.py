# Native executable oracle. Run with python -m tools.projectile_oracle.voxel_launch
...
def run(dx,dy,dz,speed=100,arcing=False,lobber=False,floater=False,voxel=True,vertical=False):
    ...
    u.mem_write(PTYPE+0x236,bytes([voxel]));u.mem_write(PTYPE+0x2c0,bytes([vertical]))
    ...
    return dict(delta=[dx,dy,dz],speed=speed,voxel=voxel,vertical=vertical,velocity=list(struct.unpack('<ddd',raw)),bits=[f'{b:016x}' for b in struct.unpack('<QQQ',raw)],pitch=r32(u,SP+0x80)&65535,max_speed=r32(u,BULLET+0x110),success=u.mem_read(SP+0x27,1)[0],calls=seen)
rows=[run(*delta,voxel=vox,vertical=vert) for delta in [(500,0,-300),(500,0,0),(500,0,300),(0,0,-300),(0,0,300)] for vox in [False,True] for vert in [False,True]]
Path(__file__).with_suffix('.json').write_text(json.dumps(rows,indent=2), encoding='utf-8');print(json.dumps(rows,indent=2))
