# Native executable oracle. Run with python -m tools.projectile_oracle.fireat_launch
...
def run(dx,dy,dz,speed=100,arcing=True,lobber=False,floater=False):
    ...
    return dict(delta=[dx,dy,dz],speed=speed,arcing=arcing,lobber=lobber,floater=floater,velocity=list(struct.unpack('<ddd',raw)),bits=[f'{b:016x}' for b in struct.unpack('<QQQ',raw)],pitch=r32(u,SP+0x80)&65535,success=u.mem_read(SP+0x27,1)[0],calls=seen)
rows=[run(*delta,speed=speed,arcing=arc,lobber=lob,floater=floater) for delta in [(500,0,0),(500,300,0),(500,0,-200),(500,0,300),(20,0,300),(-500,300,0),(-500,-300,0),(500,-300,0),(0,500,0),(0,-500,0),(0,0,0),(5000,0,0)] for speed in [0,10,100] for arc,lob,floater in [(True,False,False),(True,True,False),(False,False,False),(True,False,True)]]
Path(__file__).with_suffix('.json').write_text(json.dumps(rows,indent=2), encoding='utf-8');print('original FireAt plus BulletFire cases',len(rows),'persistent vectors equal',sum(r['success']!=0 for r in rows),'failed launches',sum(r['success']==0 for r in rows))
