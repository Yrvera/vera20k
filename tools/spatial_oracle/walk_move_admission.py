...
def query(row):
 ...
 call(0x49F2F0,0,[])
 u.mem_write(ACTOR,dwords(VT));u.mem_write(VT,bytes(u.mem_read(0x7EB058,0x600)));u.mem_write(VT+0x70,dwords(ACTION));u.mem_write(ACTOR+0x6C0,dwords(TYPE));u.mem_write(ACTOR+0x21C,dwords(HOUSE));u.mem_write(ACTOR+0x684,b'\xff');u.mem_write(ACTOR+0x9C,dwords(*row.get('xyz',[5*256+64,5*256+64,0])));u.mem_write(ACTOR+0x8C,bytes([row.get('on_bridge',False)]));u.mem_write(ACTOR+0x74,bytes([row.get('marked',True)]));u.mem_write(ACTOR+0x90,b'\x01')
 u.mem_write(TYPE+0x5B4,dwords(row.get('movement_zone',4)));u.mem_write(TYPE+0x67C,dwords(0));u.mem_write(TYPE+0xC8D,bytes([row.get('move_to_shroud',True)]))
 call(0x75AA90,LOCO,[]);u.mem_write(LOCO+0xC,dwords(ACTOR));u.mem_write(ACTOR+0x674,dwords(LOCO+4));u.mem_write(LOCO+0x14,dwords(1));u.mem_write(LOCO+0x28,dwords(*row.get('head',[0,0,0])))
...
if __name__=='__main__':
 finish_vectors(generate,Path(__file__).with_suffix('.json'),provenance=lambda:provenance(
  scope='Original Foot4DE1D0 ordinary Infantry/Walk Cell-click destination resolver, including original coordinate/ShouldBeOnBridge, shroud, CanReachZone, full FNPC and its projection. Input action selection is supplied; synchronized executor and AStar are outside this comparison.',
