...
that child. Hardware execution breakpoints observe PE entry and WinMain entry.
...
breakpoints=[0x7cd80f,0x49f0e0,0x49f190,0x6bb9a0]
def arm(c,which):
        ...
        if event.code==3:
            ...
            if er.code in (0x80000003,0x4000001f,0x80000004,0x4000001e):
                ...
                if c.eip in breakpoints:
                    ...
                    payload['captures'].append({'eip':hex(c.eip),'fpcw':hex(c.fp.control),'region_address':'0x89f670','region_hex':region.hex(),'original_instruction_bytes':text_bytes.hex()})
                    if c.eip==0x6bb9a0:
